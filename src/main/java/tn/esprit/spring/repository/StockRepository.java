...
public interface StockRepository extends CrudRepository <Stock,Long>{
	...
	@Query("SELECT new tn.esprit.spring.DAO.entites.Stock(s.qteStock,s.qteMin) FROM Stock s")
	List<Stock> retrieveqtestockandqtemin();
	 
	@Query("SELECT new responses.Fournisseurprod(p.fournisseurs,p.libelleProduit) FROM Produit p WHERE p.stock.idStock =?1  ")
...
//	List<Fournisseur> retrievefournisseurparstocks(@Param ("test") long test);
	...
	@Query("SELECT p FROM Produit p INNER JOIN p.fournisseurs f WHERE f IN (?1) AND p.stock.idStock =?2  ")
	List<ProduitEntity> retrieveproduitsparfournisseursetstocks(FournisseurEntity p,Long test);
	
	@Query("SELECT s FROM Stock s INNER JOIN Produit p ON p.stock.idStock=?1 AND p.libelleProduit=?2")
